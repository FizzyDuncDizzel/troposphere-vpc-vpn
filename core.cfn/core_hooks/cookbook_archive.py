...
import runway.cfngin
from runway.cfngin.lookups.handlers.default import DefaultLookup as default_handler
from runway.cfngin.lookups.handlers.output import OutputLookup as output_handler
...
def package_and_upload(provider, context, **kwargs):  # pylint: disable=W0613
    ...
    cookbook_relative_path = default_handler.handle(
    ...
    if kwargs.get('chef_config_bucket_output'):
        chef_config_bucket = output_handler.handle(
        ...
        if LEGACY_STACKER:
            ...
            chef_config_bucket = xref_handler.handle(
                kwargs.get('chef_config_bucket_xref'),
                provider=provider,
                context=context,
            )
    bucket_key = default_handler.handle(
    ...
    if kwargs.get('additional_dirs_relative_paths'):
        for i in kwargs.get('additional_dirs_relative_paths'):
            add_dir_to_archive(
                ...
                os.path.normpath(
                    os.path.join(os.path.abspath(os.getcwd()), i)
                ))
